...
<link rel="stylesheet/less" type="text/css" href="{{ RUTA_URL }}/public/css/styles/matapp-registroUsuario.less">
        ...
        <div class="col-12 col-md-6 text-dark bg-white d-flex justify-content-center" style="height: 100vh" id="derecha">
            ...
            <div class="register-container w-100 w-md-50 d-flex justify-content-center" style="margin-top: 15vh">
                ...
                @if (isset($_GET['errores']) && !empty($_GET['errores']))
                    @php $errores = json_decode(urldecode($_GET['errores'])); @endphp
                @endif

                @if (isset($_GET['success']) && !empty($_GET['success']))
                    @php $success = urldecode($_GET['success']); @endphp
                @endif

                @if (isset($_GET['registro_error']) && !empty($_GET['registro_error']))
                    @php $registro_error = urldecode($_GET['registro_error']); @endphp
                @endif

                @if (isset($_GET['login']) && !empty($_GET['login']))
                    @php $login = urldecode($_GET['login']); @endphp
                @endif

                @if (isset($_GET['email']) && !empty($_GET['email']))
                    @php $email = urldecode($_GET['email']); @endphp
                @endif

                <form class="p-3 rounded-2" id="form-register" action="{{ RUTA_URL }}/registroUsuario"
            ...
            <div id="carouselExampleAutoplaying" class="carousel slide" data-bs-ride="carousel">
                <div class="carousel-inner">
                    <div class="carousel-item active">
                        <img src="{{ RUTA_URL }}/public/img/registro/i2.jpg" class="d-block w-100"
                            alt="..." />
                    </div>
                    <div class="carousel-item">
                        <img src="{{ RUTA_URL }}/public/img/registro/i1.jpg" class="d-block w-100"
                            alt="..." />
                    </div>
                    <div class="carousel-item">
                        <img src="{{ RUTA_URL }}/public/img/registro/i3.jpg" class="d-block w-100"
                            alt="..." />
                    </div>
                    <div class="carousel-item">
                        <img src="{{ RUTA_URL }}/public/img/registro/i4.png" class="d-block w-100"
                            alt="..." />
                    </div>
                </div>
